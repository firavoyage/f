const flair = ({
  ...
  // utility fn
  const { is, has, keys, values, each, flatten, map, generate } = f;
  ...
  const theme = map(
    Object.assign(themedefault, _theme),
    ([type, variables]) => [type, flatten(variables, "-")]
  );
  ...
  const glossary = Object.assign(glossarydefault, _glossary);
