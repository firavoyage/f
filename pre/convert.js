(() => {
  ...
  const render = (text) => {
    ...
    const setupMathRules = (md) => {
      ...
      md.renderer.rules.math_inline = (tokens, idx) =>
        renderMath(tokens[idx].content, "c", false);

      md.renderer.rules.math_block = (tokens, idx) =>
        renderMath(tokens[idx].content, "c", true);
  ...
  const convertMarkdownToSlides = (markdown, config = {}) => {
    ...
    const isBlockElement = (node) =>
      ...
      ["BLOCKQUOTE", "DETAILS", "PRE", "FIGURE", "TABLE"].includes(
        node.tagName
      );
  ...
  globalThis.convert = convertMarkdownToSlides;
})();
