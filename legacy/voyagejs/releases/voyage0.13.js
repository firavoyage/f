...
let voyage = {
  ...
  call(componentid) {
    let { counter, states } = voyage;

    counter.componentid = componentid;
    states[componentid].stateid = 0;

    const { component, properties } = states[componentid];
  ...
  createComponent(component, properties) {
    ...
    init(states, componentid, { component, properties });
  ...
  refState(initial) {
    const { init, isnt } = voyage;
    const { updateState } = voyage;
    const { defineProperty: watch, assign: put } = Object;
...
const { counter, createPoly1, createPoly2, counterMacro, hello } = examples;

voyage.run({ component: counter, parent: "body" });
voyage.run({ component: createPoly1, parent: "body" });
voyage.run({ component: createPoly2, parent: "body" });
voyage.run({ component: counterMacro, parent: "body" });
voyage.run({
  component: hello,
...
// - states[componentid][stateid][key]
// store global
