...
in memory call is philosophically sound (one app one port, instead of one process one port), but it does not offer any practical value. actually it makes things much worse, as it's the backend that serves everything at the end of the day, not the dev server.
...
also, i would reconsider if `{ref.palette.success}` is needed. maybe `"ref.palette.success` is enough. it cant be ambiguous. ref if existing, otherwise raw.
