# scratch pad

plan:

<!-- think harder. -->

first, prompt.

ask the questions, one a time.

---

things:

watch silence gamedev (find inspirations about promotion)

read no longer human

diy penacony origami bird

read and watch: feel cursor (claude code) vs simple chat. cursor (others are similar) built in prompts for automation. (add explanation.)

practice english calligraphy

switch context as i like.

chat:

how to describe food in english

appreciate the lyrics of the wind of change

innocent:

how to live with time. schedule or not. manage time. want to master, become servant? use a schedule or not. how to be with time. not manage or be controlled not productive, but feel right. why/how schedule. scratch pad. projects/weekly/reading/watching/intentional surfing.

thoughts on nat.org

what would llms prefer. e.g. do over dont, desc over exempl.

know llms' personality, 5 scenarios, 3 tasks for each.

when would strong ones outperform weak .. (when to choose fast/thinking)

what do you think about employing children with spare money in parenting (too much, too few)

sunrising and sunsetting in the real world ::heart:: the gradients in the sky be anytime anythere (proj. all)

organize sub for intentional surfing.

how to read (no longer human) (manage time, pursuit) i wonder what determines what i do

how to self learning jp with biao ri.

what could i learn to create an indie game

how to learn ui (ux) design

what could i do when im free

how to perceive the world. how to archive my perception.

breakdown classify writing style, biaodafangshi. what are the (sub) concepts of writing style

how to ~~learn from scratch and others to~~ design a sweet indie game engine with handy utilities. ~~how to use threejs in the simplest way to implement my 2.5d indie game design~~

how to design the pieces of my indie anime fantasy style self moving chess video game with moba elements like hp, weapons and abilities, and characters from either original stories or popular works among japanese otakus

how to be a content creator as one who prefers thinking over expressing

how to create vivid math tutorials as an anime otaku oriented channel

how to create a design system as an indie developer of tooling utilities, with variations including a retro theme imitating the real world, like iphone os 4, a cozy theme of soft shadows and simple gradients, like google reader and gmail legacy, a modern stylish theme of large paddings, flat elements and solid colors, like material design 3, and maybe more themes in the future.

what may an author's personal style include, thinking top down

what's the writing style of aops, tk td, how to learn from

explain kuaihuo

how to create a design system as a big company

how is a design system created in big companies. use a verb for each phase or step. storybook, example sites, what could llms help

how do software engineers in big companies go vibe coding

how to avoid philistine, short sighted, clickbait, boring, low level psycho, and use longevity rush

the pursuit of human

how to pursue wisdom.

what is everyone doing now in the real world

i wanna live better and create great works. the mindset. define great.

translate "返老还童"

human: wisdom strength boldness wealth freedom
things: simplcity, elegance, beauty, purity, abstraction

what's the loss of adultship in the modernized world. what could be learned from children.

week is the metaphor of working. en: 7, zh: 10. defined beyond nature by human.

i would like to learn the high level method of thinking ("simplify")

i would like to learn from smart self learners

i would like to have some journaling prompts to help me think, decide, and do right things (a perfect move)

prompt: fira and innocent girl, natural (simple!), direct (info or task)

innocent girl: have a big picture. (start fresh, review and polish myself. no need to strictly keep the original wording.)

journal:

dont think about that anymore. mindfulness, the precious thing of mine. way more important than. (thrill: problem solving, "ctf". mfn: transl) how did i? cn guys. (here we have to waste a lot of energy to get to the starting line and be sane.)

blank cards, plant, lifesize pillow, doll from menci.

misc:

zhihu following list export. favlist export enhanced.

project all::map have, everywhere.

pure math contents <- advanced topics and triks on the cheetsheet (ocr)
gen and revise, compare and summarize the style. aops. apply the style.
set the narration style when translating. "for enthu, memories, ref."

<!-- wanna be a design engineer, an indie gamedev, a teacher of the 10th grade learner. -->

fira's company careers: hidden. (underaged sex service for fira, voice actor like bingtangio, politics freedom human rights, army with atomic bomb engineer, children labor)

org berryberry fav (n archive, ofc)

https://space.bilibili.com/1156993272/favlist?fid=3685161172&ftype=create

https://space.bilibili.com/1156993272/fans/follow?tagid=345842753

12 03

iwlt have a big picture of human life. what are they doing.

the way of life. decide what and how to do. "what could i do, i have a free weekend/ a few hours spare time" (do something meaningful instead of random surfing)

time/schedule

think

create a software project in big companies / as an indie dev / of llms.

learn: tell, recommend

design journal (the first prior. see doubao/trae the vid), games

upd: inspired by innei, i couldnt wait. i want it this week. 

coding is easy. but idk what i want. philistine?

maybe i should focus what's important. the journal app. the way of creation.

iwlt learn the methodology to create a big sfw prj with llms (context len, detail level, how human think n do)

i could read light novels (not hardcore ones), and watch bangumis.

hug the mutsumi. 

play go?

archive web. (first priority now!)



japanese biaori

https://space.bilibili.com/1156993272/fans/follow?tagid=345842753

https://space.bilibili.com/413703/video?tid=0&special_type=&pn=1&keyword=&order=click



## carte

what about using grayscale colors everywhere and vivid colors only on anime characters. (what's the methodology in animation.)

## journal

after carte.
