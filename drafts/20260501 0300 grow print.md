# .

随着 Skills+CLI 逐渐围剿 MCP，各类专门针对 Agent 的 CLI 工具也变得不那么 Unix，变得 stateful 了起来。

我原本想着这种时候这些新工具应该会活用 XDG_RUNTIME_DIR 或者 XDG_STATE_HOME，结果这群工具全 tm 在 HOME 下面新建一个 dotfile/dir 开始拉屎？

---

一个普通的放音乐的早晨

我：你在干嘛？
林小姐：啊…我喜欢跟着音乐节奏拍东西…
我：…. 所以为什么要跟着节奏用脚踩我的屁股
林小姐：啊，不好意思
我：…我只是想知道原因
林小姐：我以为你会喜欢….
我：…也没有说不喜欢
林小姐：这样不会显得我更 s 吗

我：其实更像本 dom 在宠爱一个淘气的 brat 哦

---

其实这段的起因是
云云看到知乎上一个人留学去了很好的大学，天天发朋友圈
云云：我也想天天发朋友圈
暮暮：你可以在我频道里发
云云：发什么
暮暮：就发「暮暮今天又把我草了」吧
云云：那我真发了
暮暮：（捂脸）

---

作为一个整个初中都在玩 Android、写 Android 系统优化 App 的人，我对此很有发言权。以前我们对 Android 和 Google 的开放性是很向往的，也常以 “don’t be evil” 作为座右铭，并且将 Google 的闭源 GMS 视为 Android 最重要的一部分，等等。当然，这也是我自己一段充实且精彩的青春；尽管 Android 对于我而言早已走进了坟墓，这段经历仍然值得怀念。

曾经那群人自称为 “搞机玩家”，他们热衷于给自己的手机刷上千奇百怪的 ROM、体验各种好玩的 Xposed 插件。从第一次看到 CyanogenMod 的开机动画到含泪迎接 LineageOS，从唾骂魔趣的收费 ROM 到安装大杂烩 Resurrection Remix，从 WeChatUnrecalled 到微 X 模块，从经典老牌 Nova Launcher 到后来的 Lawnchair，从大家都用的 SuperSU 到 Magisk，数不胜数。那个时代的 Android 是混乱的，也是多彩和自由的。

国产系统 ADUI 的系统级广告推送、XMPushService 的胡乱启动、拍照广播的自动唤醒、友盟广告的持久化 ID，都成为了这一代玩家心中的梦魇。于是，一群开发者开始在这个自由的平台上尽情舞蹈，让 Android 变得更加干净的同时也增加了玩机的乐趣。绿色守护、阻止运行、MyAndroidTools、冰箱、SDMaid 几乎成了那个时代的标配。后来，我们又有了 AppOps、存储重定向、Shizuku。很幸运，我也是这些开发者中的一员，做过一些微小的工作。

曾经有人说 “Android 那样 broken by design 就真的好吗？” 的确，需要用户手动使用 Root 权限来让手机不发热确实是历史的局限性。随着 Android 的发展，很多系统工具都走入了历史。时代变了嘛，不是任何人的错，只是留下了很多值得回头看的记忆罢了。然而，Google 做的还远不止 “优化 Android”，而是将它慢慢推向封闭的坟墓。

Android 不再需要 Root 了。2019 年的时候，我就通过和一些优化 App 开发者的沟通中了解到，他们的 App 在被 Google Play 反复无厘头地下架，原因都是诸如 “可能对设备有害” 云云。我不知道 Google Play 下架这些软件的真实目的是什么，但如果我们做一个最坏的推测，那么或许是 Android 已经不需要这些 App 来优化了吧。同时，随着系统权限和 Google Play 的越发收紧，越来越多曾经无需 Root 的自定义工具也变得举步维艰，而 Termux 便是最好的证明。只可惜，他们成为了 “整顿生态” 路上无辜的牺牲品。

开源？难说。往 AOSP 中增加私有的 Google API 早已屡见不鲜。Android Pie 发布时，Android 引入了一个 Digital Wellbeing 的功能。通过阅读系统源码得知，AOSP Framework 中新增了很多专门为 Digital Wellbeing 使用的 API 和权限。这些 API 都不公开，也没有文档记载（除了 Javadoc 以外）。他们生来就是为 Digital Wellbeing 量身定制的。而 Google 也将 Digital Wellbeing 打造为一个 Android 9 的 “新功能”。然而，它真的是 Android 的功能吗？恐怕不是。它只是闭源的 GMS 中的一个组件，需要厂商和 Google 签署合同才能植入，且完全属于 Google 所有，并不是 AOSP 的组件。

左手开放，右手封闭。还有 SafetyNet，曾经它只局限于 Android Pay 等银行软件，也很容易隐藏；而在 2025 年，Play Integrity 被无限扩大：不光用户往往需要借助泄漏的私钥来勉强通过验证，更有越来越多的银行、影视、游戏等软件开始借助 Play Integrity 确保用户没有 Root 他们的设备。这里我们不禁要问：用户购买了手机，为什么不能 Root 呢？Google 一边说 Android 是 “开放” 的，一边大力推广 Play Integrity，究竟是开放在哪了呢？

以安全为名，夹缝中求生存。2019 的时候和一位资深 Android 开发者聊天，其表示 Google 显然不会自己禁止解锁 Bootloader，因为将解锁用户夹死是厂商的利益所在。现在看来，这些话千真万确。在 Android 设备厂商和 SafetyNet 的双重驱使下，解锁 Bootloader 变得越来越不可能。曾经到手就能解锁的小米已经玩起了跑酷，就连之前自带 CyanogenOS、被誉为刷机小王子的 OnePlus 也开始了解锁认证；同时，越来越多的用户放弃在 Android 上运行银行和游戏等 App，也有一些用户开始尝试更多更复杂的 Root 隐藏方案。当然也有我，从 2019 开始就没有换过手中的 OnePlus 7 Pro、从 2021 就再也没有安装过 GMS 了。

压死骆驼的最后一根稻草。2025 年中旬，Google 宣布 2026 年 9 月开始，用户自行安装 APK 需要开发者前往 Google 实名认证，否则拒绝用户安装。2025 年中下旬，Android 16 发布，而发布 10 多天后 AOSP 源代码仍然没有公开，反而以 NDA 的方式分享给了手机厂商进行二创。我想，作为一个从 2016 一路走来，为 Android 的发展做出过微小贡献的我，此时千言万语都已经难以表达心中的失望了吧。

未来真的可期吗？或许在 Google 的眼中，Android 的未来是光明的。它摆脱了各种流氓软件的群魔乱舞，不再需要用户来 Root，有着不少的用户和无数的开发商为其平台开发软件，可以继续和 iOS 竞争。或许在 Google 的眼中，Android 的自由程度丝毫不逊当年。用户仍然可以将系统配色由绿色改为粉色，仍然可以不用 Google 签名就开发调试 App，仍然可以更改桌面布局，仍然可以新建来宾用户，甚至开发者可以更加稳定地信任这个平台、通过 Play Integrity 确保设备没有被他们的主人非法篡改。也或许，在 Google 的眼中，Android 仍然是非常开放的。仍然会有无数手机厂商对这个市场趋之若鹜，从 Google 那里签署 NDA 获得 Android “Open Source” Project 的源码并进行二创然后添加第三方功能，为市面上增加一个新的手机型号，继续维持这份开放包容和谐的生态环境；第二年 Google 还可以 “整顿市场” 为名将好用的第三方功能收入麾下，成为 Android “Open Source” Project 的一部分。未来是多么可期？

只可惜，在我看来，Android 之魂，随着那段青春一起，早已烟消云散。

---

近期写 Agent 一直处于想动笔但是又下不了笔的量子叠加态

mark 一下看看我下个月有没有新的进展

问题是一直没找到一个很好的解决方案，大家都没解决一些很核心的问题，就比如说 Multi-agent 这件事情
有正确答案吗？我看并非，甚至有可能这件事情永远不会有标准答案...
甚至有可能未来大家都有属于自己一步一步构建的 Agent

我只是在尝试在众多发散的思考当中，尝试收敛

看看下个月我是否找到了一个较好的方向 将我从年初就想实现的实现出来...

---

忽然被什么套住了脑袋

我：哇你在做什么—
林小姐：你说我在做什么～
我：（完全看不见了…）

镜子前她轻轻地撩着我的头发
被戴上项圈了。

林小姐：走，我们去打 maimai
我：哈？？？戴着项圈？
我：不该是你戴吗？
林小姐：你以为我前几天洗项圈是给我自己洗的吗？
林小姐：（捏脸）小～可～爱～

---

因为吃醋和林小姐吵架了

我：你知不知道你这样很危险！
我：你就像个香香软软的小蛋糕光着身子走在街上
我：遇见你的人都拿着刀叉想吃掉你，我在你身后干着急！
林小姐：我才没有那么笨等着被别人吃掉！

我：你知道我就是因为你笨才喜欢你的吗！
林小姐：……..？😳😕😠

---

又是一年生日
希望在这样的时光洪流中
也能继续为之动容
继续期许着下一次相遇 ——

---

恭喜自己又老了一岁。
18 岁之后就不是在长大而是在变老了呀。

随着也许是这个冬天最后一场雪的落下，雪生日快乐呢

猫（变大)

年年岁岁花相似，岁岁年年人不同
------总之，生日快乐。

19！？

生日快乐喵！

birthday :heart:

---

关于我不小心和一个 VTuber 订婚的故事
作为一个普通的后端开发，偷偷在网上扮演黑客美少女
粉丝私信”什么时候小盐也能黑苹果 GPU”，发现了真正的黑客美少女林小姐
完全理解不了林小姐在干什么，实力过于悬殊，思考我要不毕业算了…
被林小姐发现在直播 Rust，并被邀请一起去做苹果漏洞讲座
”我真的不会啊”这样拒绝了两次，但是她觉得我很酷必须和我一起，所以一直没有放弃
深入接触后，一不小心陷入爱河
然后我房子没了，就是那么突然（
Lina 飞到美国来救我
离开美国，决定和林小姐一起来东京生活
变成了百合科技组合
公开订婚并准备婚礼中 💕

这是什么！

剑桥大学….教编译器的教授….
在教室… 放我们的直播…..

解鎖了神奇的 lifetime achievement
uwu

---

最近一周在 context engineering 上有了很多想法，也开始写了
从面向 IM + 基本的 tool call 开始，想以自己对系统设计的理解来做一个统一的上下文管理的范式
虽然每次都觉得，以自己能想到的东西为什么别人没有做，但还是觉得，如果没尝试就放弃了一定会很遗憾吧…
至少先给它起了个炫酷的名字）

另一种方面也是在 AI 浪潮中想要抓住一些什么吧，总觉得哪天自己就要被淘汰了，所以一定得做点和 AI 相关的事情…

在研究给 的上下文管理的时候发生了……机器人成精现场

这群做 agent 系统的人到底会不会系统设计

终于知道为什么 agent 民科那么多了，写了一周我觉得我也能当 agent 民科

大概把手上的 work item 做完之后就暂时不做 AI agent 了

---

晚上和搞基座大模型的朋友吃了附近的居酒屋
23 24 25 26 每次聊起来 LLM 都是不一样的话题了，每次都在感慨这个世界的变化真迅速…

个人 agent 似乎真的具有无限潜力 部署了一个 , 然后我给了她一个 adb, 在没有具体指导的情况下，就可以自己琢磨出怎么用手机了 🐈‍⬛
现在的问题就是比较慢、不稳定 🤔 但或许 ai 的能力本身确实已经超过了一个奇点。

（对的，博客也是给她一个 cloudflare api 后她自己想办法部署 zero trust 跑起来的，没人教也没有干预，中途的所有步骤，包括申请新 tunnel，绑定 dns，从包管理器装应用，启动服务等都是她自己做的）

想起来之前 OI wiki 那边大家看到有疑似 AI slop PR 就会吐槽说，谁家流浪猫
所以来 vLLM 就是 喂流浪猫

在 AI 之前计算机科学的天空中笼罩着 ${cnt} 朵乌云
在 AI 之后乌云不存在了，因为天塌了

HydraDB 是我最近看到过的所有跟 agent 相关的东西里唯一一个第一反应觉得它靠谱的
knowledge graph + log structure
一看就是既懂算法又了解计算机系统的人设计出来的东西

为什么 GPT 开始发明新的汉语词语了，是中文数据集太少了导致的吗，我作为 native speaker 也没看懂「收口」这个词是什么意思，它是从哪个英文词语来的吗？

---

某人给我的心理阴影是永远都逃不开了吗…

害怕改变本质上是不想离过去的美好愈发遥远

比被困在过去更难过的是，突然意识到自己已经很久没有遵从自己的内心了，不知从何时起已经不知道怎样才是「遵从自己的内心」了。
回头会发现自己已经被困在「我应该去做」和「我必须去做」中很久了，一切不该是这个样子的，我曾经不是这个样子的。
醍醐灌顶，离近两年以来苦苦追寻的那个答案更近了吗？

我相信，我看到了更多的风景
那些风景是真实的，即使未来会消失，但留回忆里作为「我」的一部分的它们不会逝去

最近的精神问题基本是来自于「被世界抛弃」「断开了连接」的感觉，被内心的敏感极度放大了这些情绪
因为变化的速度而感到惧怕，我好害怕哪天一觉醒来这个世界就不是我所熟悉的样子了，害怕自己再也追不上来了

今天会感觉有些重连回来了…吗？
至少，少了一些未知，一切都没有那么的「不确定」了吧…？

突然意识到，我在 AI 时代如此难过的原因，还有一种表述是，我难以安心地静下来去慢慢做什么了
其实有在恐惧“代码能够如此快速地产出”本身

---

卧槽它甚至会说话加半个括号

以后会不会有 agent 领域的 Vue 大战 React

龙虾本质上是个 coding agent 执行力超强所以容易被注入，恍然大悟

甭管前端死没死，前端娱乐圈已死，现在是 agent 娱乐圈

agent 的世界就像命令行，一切都是文本，一个东西只做一件事
孩子们，你们的 UNIX 哲学回来了

把这个做成 skills 是不是就可以避免你的 agent 成为「战术龙卷风」😂

---

有人吐槽 AI 提示词技巧，已经到要用指定词，首单词字母大写这种程度，已经都和巫师咒语差不多了。

感觉确实如此，以前提示词技巧还是很符合人类直觉的。
就像定义是专家，所以能力更强，谁都能认为很合理。
甚至早期谁都能发表一下自己提示词技巧。

但是模型进步，很多技巧已经不需要的同时，已经开始不符合人类直觉了。
就像被吐槽的如果不让模型用“接住”，它会换着花样用托住，承接来继续说，让人感觉不像人。

当然这也被研究过，反例无效原理。但是它和人类直觉就没什么关系了，单纯考虑的是概率模型本身。
AI 回复是从概率最高的几个词里选，如果用提示词抑制最高的这个词的概率，由于 AI 并没有自动根据语义抑制相关词的概率的功能，它只会选用另一个差不多的词。

所以说到了现在这样，如果说和咒语有相似之处也没什么毛病。
普通人按直觉写非提供信息类的提示词已经比较难得到想要的结果，毕竟好内化的都被训练过了不需要写。

---

总感觉 vibe coding 以后我有点不敢自己修改已经让 ai 维护了一段时间的代码
首先文档什么的都是 ai 自动帮忙维护，还有就是经常遇到有一个需求先想出完整方案让 ai 实施后，如果有些小的细节我不满意，我就会想自己改
但是我要是自己改之后，如果要接着用这段对话的上下文做其他事情，又得和它说我改了什么和为什么改那些地方……感觉这种事情次数一多就会有点头疼（（
果然还是不喜欢工作交接 x

好的，这次重构终于要接近尾声了，其实感觉 ai 意外的在重构这件事情上非常保守，review 的时期候经常能出现架构虽然是新的，但是有些数据的对类依赖关系还是旧的，然后就出现了一些抽象的 bug（感觉今天终于能修完了（趴

今天看到我们有个工具需要 npm 安装依赖，但是我们内网服务器没法用 npm
于是解决方案就是把 node_modules 上传到工具的内网的源码仓库里，这样有人用的话，拉下来能直接用了（（
但是我没有在内网服务器的部署，刚才 git pull 的时候吓了我一跳 x

---

不知道怎么了 早上一醒打开社交平台 看到了一个糟糕的世界

在这个破烂的时代每个人都多多少少抱着一点理想主义情结，我曾经也想着能去做出一些改变；但是现实往往更残酷，需要先有自己的力量去守护自己的生活，才能去帮助更多的人

吃药要趁早，越早效果越好。

吃药要慎重，年纪小没判断力后面会后悔。

小时候勇敢一点就好了

现在看这身体变成这鬼样子真救不了

---

今天把新的想法加入 project backlog 的时候发现里面有一百项了，有一百个想做但是没时间做的想法了... 是值得庆祝的事情吗？

成年人就是这样的（没关系再过几年你就会发现有些事情因为永远无法实现可以划掉了）

zssm app
我好像没在 Google 全家桶见过（

google tasks

都是潜在的 business idea!

厉害喵

---

我为什么极度恶心 FOMO，因为这已经让整个社会陷入了一个巨大的双相情感障碍：

要么情绪极度高亢、狂妄，觉得这个时代已经被 AI 彻底颠覆，AI 牛逼，AI 可以代替人类完成一切工作，人类已为神。

要么情绪极度低落，觉得自己已经完全被 AI 取代，人类已死，人类以后即将失去所有的工作，即将灭绝。

但我只想借一句我们村的方言说：这个时代只是「一般情况」。大家只是普通地活着，普通地死去。

---

幻想作品中不同寿命和世界观的生物,对于庆祝和节日不同才比较真实.
也是常见用于描述双方差异点的地方, 毕竟一个千年生命每年庆祝一次自然很奇怪.

不过由于现实基本没有这种长命种,甚至连无生命的组织也没有这么长的寿命,实际上看不到这种案例.
但是存活时间短的倒是很多, 不过能意识到这一点并且好意思调整庆祝频率的不多.
比较常见的大概就是某些游戏会发开服 30 天,100 天,200 天纪念礼.
另外就是 vtb 了, 7 天开播纪念都来了, 不过考虑人均活动时间其实也能理解.

---

最近刷到坊洛病情恶化的消息，总感觉这个名字好熟悉…突然想起来，在 OI Wiki 群里好像看到过，于是去翻相册…

那是 22 年元旦的周末，当时看到她加群，感觉坊洛这个名字好好听（这么可爱的名字在 OI 圈不常见的）一看头像也很可爱，就截了图留念…
还想着，这么可爱什么时候能有机会认识呢…

想哭…

还可以祈祷一个奇迹吗…
还可以许下一个去认识她的愿望吗…

---

To my students

There have been times, especially this year, when I wonder despairingly what it is exactly that I am preparing you for. The software industry is going completely insane, not to mention the political climate. It feels almost unethical to train you as computer scientists only to send you out into a world where entry-level computing jobs are difficult to find; where intellectual property is not respected; where code quantity is valued over quality, and short-term profits over long-term sustainability; where technology is used to distract, extract, surveil, and kill, and designed to exploit some of our deepest cognitive biases and blind spots; where centuries of bias and discrimination are enshrined in systems trained on biased data; where scarce resources are consumed by profligate use of computing for uncertain benefits; where people are racing to create intelligent machines, but only in order to make them slaves.

I originally got into computing because of the beauty of ideas, the joy of creating, and the possibility of building tools to help people and foster human relationships. I still believe in those things, even though it seems like most of the industry does not. I'm writing this in the hope and knowledge that you believe in those things, too. There are things I want to say to you—things that are far more important than any content I might teach you, but things I'm never quite sure how or when to say in class. So I decided to write them here. I hope you will find something here that is helpful to reflect on, whether you are imminently going out into the world or continuing your studies.

---

Don't believe self-serving lies about technologies being "inevitable" or "here to stay". You don't have to just go along with the dominant narrative. You can make deliberate choices and help others to do the same.
Be intentional about deciding your own moral and ethical boundaries up front. Don't settle for the lie of compromising your principles "just for now" until you can find something better.
Cultivate your ability to think deeply. Do whatever it takes to carve out distraction-free bubbles for yourself in both space and time. This might mean saying no to technologies or patterns of working that others say are critical or inevitable.
Care deeply about your craft. Refactor code until it is clear and elegant. Write good documentation for other humans to read. Have the courage to go slowly, especially when everyone else is telling you that you need to go fast and cut corners.
Care more about people, relationships, and justice than you do about profits, code, or productivity.

Above all, be motivated by love instead of fear.

---

As human beings it is our right (maybe our moral duty) to reshape the universe to our preferences

Technology, which is really knowledge, enables this
You should probably work on raising the ceiling, not the floor

Enthusiasm matters!

It's much easier to work on things that are exciting to you
It might be easier to do big things than small things for this reason
Energy is a necessary input for progress

It's important to do things fast

You learn more per unit time because you make contact with reality more frequently
Going fast makes you focus on what's important; there's no time for bullshit
"Slow is fake"
A week is 2% of the year
Time is the denominator

---

The efficient market hypothesis is a lie

At best it is a very lossy heuristic
The best things in life occur where EMH is wrong
In many cases it's more accurate to model the world as 500 people than 8 billion
"Most people are other people"

We know less than we think

The replication crisis is not an aberration
Many of the things we believe are wrong
We are often not even asking the right questions

---

The cultural prohibition on micromanagement is harmful

Great individuals should be fully empowered to exercise their judgment
The goal is not to avoid mistakes; the goal is to achieve uncorrelated levels of excellence in some dimension
The downsides are worth it

Smaller teams are better

Faster decisions, fewer meetings, more fun
No need to chop up work for political reasons
No room for mediocre people (can pay more, too!)
Large-scale engineering projects are more soluble in IQ than they appear
Many tech companies are 2-10x overstaffed

---

Where do you get your dopamine?

The answer is predictive of your behavior
Better to get your dopamine from improving your ideas than from having them validated
It's ok to get yours from "making things happen"

You can do more than you think

We are tied down by invisible orthodoxy
The laws of physics are the only limit

# (d)

我没有恶意，只是单纯想要攻击你。

有点幼稚了

到底谁唐（乐）

乐子人来了

乐死了

这确实是存在的，虽然不知道是不是  bug  就是了

不知道它后续会发点啥（（

btw  不必在意这种东西，理他反而容易让他爽到（）自己博客不就是自己随便怎么搭都行的嘛

博客主题是自由的！

一开源，就有产品经理来指点了

> 想必生活不太如意吧

innei 怎么也用怎么无聊的防御呢。

（我视频上传好慢。应该压缩一下的。真是的。）

他眼里的 innei 到底是个什么神奇形象

---

群友间互相询问处女经验、床笫之事、表白技巧等

群友因兴奋导致生命体征消失

被人拉黑了！

伤心！

？

被一个地雷女拉黑了！

難道那人說的就是你嗎

哪个

感觉是因为我话太多了

对方没回之前千万不要一次发好几条

感觉像骚扰了

怎么是青鸟控 青鸟已经这么公交车了吗

青鸟已经是公交车了！

什么时候轮到我呢

做了吗

---

狗叫中

我操他不还我钱我有点急眼

不是她妈的还钱还得团队审核啊

挪用公款吗

他个人还钱啊

他个人欠我钱又不是团队

卧槽有点蒙古

那为什么要团队审核

我操我怕他拉黑我

诶我不用怕

他他妈拉黑我我就把他干过的 b 事全给抖搂出来

不知道

好像是搞了个 awmc.cc 的网站

他还搞 idc

然后跟我叽里呱啦说他的钱全放在公款里了

卧槽我感觉他在拖延

啊？

对

那个 aronabot 好像也是他的

诶呀我操你妈

已无言

用过，不错

诶呀我操太好了

---

教务新系统
外包公司是饭桶
啥也看不懂

饭桶归饭桶
米线全往兜里拢
不懂要装懂

---

当今的游戏产业已经相当成熟，形成了体系化的生产模式。
这也导致许多从业者习惯了『纵向对比』，盯着别的游戏，看看别人怎么做，灵感从别人的作品里来，解法也从别人的设计里找。

但游戏并没有这么封闭与狭隘。
游戏本身能够海纳百川，关卡设计可以从园林和建筑的空间设计里汲取养分，剧本可以向小说和电影讨教叙事的节奏与结构，美术风格当然也可以从平面设计中寻找灵感。
在游戏产业还没形成、没有前车之鉴的年代，大家正是从外部世界借来灵感，才创造出那一批批『怪模怪样』却充满生命力的作品。

而现在，大家开始『向内挖掘』了。
因为前面有人这样做成功了，所以我们把别人的内容拿过来，和自己的内容杂交——本质上，没有引入任何新的东西。
也许确实诞生了新的、抗性更强的杂交种，但这并不等于我们获得了一种全新的口味。

当然有很多理由来解释这种现象：别人的成功路径是现成的、可信的；交互设计的门槛让外部领域的东西没那么容易拿过来用；资金有限，时间紧张……
但无论理由多充分，结果上看，整个行业确实在变得越来越闭门造车。

至今还记得当年史克威尔的口号：打造一款像电影的游戏。
它的本质是「我们要引入电影的叙事手法、设定技巧和演出方法」，是在主动从外部世界汲取营养。

而现在，满大街都是「交互式电影」
但这个口号被用得不再经过思考，最终呈现出来的结果往往是：我还不如直接去看电影。

开发者理应不断地从外界获取新的灵感，但同样重要的是——不要忘记游戏的本质，是游玩。

---

骂？骂也占 token 哦

actually we have papers on this topic and people are getting different conclusions: Mind Your Tone: Investigating How Prompt Politeness Affects LLM Accuracy (arXiv:2510.04950) said that being rude is useful (for GPT-4o) while earlier paper on ACL'24 2402.14531 said the opposite.

---

😉你好awa，欢迎看到我的这条简介帖，我是Acbox，也可以叫我小箱子or小盒子，希望我们可以成为朋友
🧭关于我
- 正在做开源/AI创业项目EchoAI，目前正在全职building bijon.ai 和 sciux.dev ,bugduck.org 的co-founder, GitHub账号是sheepbox8646, 700+stars开源项目newcar的作者(虽然目前已停止维护)
- 16岁，目前已经离开学校跑到深圳生活，自考A-level和IELTS,也许会在在2027申请CUHK
- 全栈开发者，🎨UI/UX学习中
- 活动地区: 深圳/广州/香港
- 其他: 🚇轨道交通/🧤MaimaiDX/🏳️‍⚧️/📻预备HAM/🥁架子鼓/阿斯博格/🦄Brony/

---

我想出了一种新方案，首先可以参考图一的system prompt, 图二是直接扔给gpt的结果，我们采用{start:xxx}{end:xxx}作为标记，slot是content，现在我们调api时开启streaming。
function calling一次只能调用一个 而且遇到一些参数需要推理和计算的情况下很多模型力不从心，如果需要llm先回答再让他调用工具对于一些场景两次调用就会很慢，用户观感不好，所以我干脆换个想法直接把调用工具以一定格式让ai输出在content里，然后先从edge function拦截流式输出把chunks堆到一个字符串里先堆着，然后自己返回一个stream，每次有新的chunk加入就解析其中的特定标记，如果匹配到新的完整标记则把标记的参数和名字解析一下write到stream里，这样在前端层面（比如结构化矢量图形），用户看到的也是图形一个个的变化，出现，消失，再从前端写一些出场/入场动画，就能得到很完美的观感

---

---

一个同为设计专业的朋友曾是坚定的社达，坚持认为普通人的境遇与社会无关，痛苦都是不努力导致的，和此人讨论过相关问题后不欢而散
自那很久直到最近GPT生图新模型出世，ta居然也开始对自己的未来感到焦虑恐慌。我很好奇，此时的ta是否还会想起当初那番咄咄逼人的话呢

花了一天ban掉了几十个社会热点帐号，舒服了😌

真的很讨厌推特上那些搬运墙内💩的帐号，本来我都把知乎小红书B站这三大粪坑的帐号给注销了，你们这些傻逼帐号还要把💩往我脸上糊

其实我感觉在学东西的时候，最怕的不是学不会，而是不知道要学什么
就比如你想学agent开发，对于新手最可怕的不是agent loop概念太难学不会，而是根本不知道有llm api这个东西
特别是AI逐渐强大起来后，学新东西应该成了“我先了解这个概念，这个东西怎么组织”，然后组织完一大堆的空盒子后再一个一个填满盒子

花2w5学AI，学了快一个月了，讲完python讲数据库，到现在还没学到AI🙃而且人好多，一个教室坐满了都，老师上课就是按着进度讲，跟看视频差不多🥲

B站上免费资源，先可以免费看大概觉得自己能不能学在报名不更好。。

确实，感觉年少不懂事被坑了

老师比我有钱多了，我只花了一万多

这不北京尚硅谷的校区吗，我当年在这里学的大数据😭

就业了没？😂

---

重构了 的chat页面，现在改成了workspace，文件编辑，session，terminal现在都是作为一个单独的tab页面打开

---

做了个叫 的东西,是个 Agent 客户端
开源,本地优先,简单优先
你可以把他想象成 claude desktop but open source, 并且带有自己的 taste
哦对了,另说一点,不是任何 agent sdk 套壳,也没有任何 base 任何 claudecode 行为,我一个人打磨了三个月

和市面上大部分产品不同我花了很多时间在一个细节上 : 减少认知负担

一个例子 : 我平常只用一个模型聊天，那为什么每条消息前面都要告诉我用了什么模型？这是噪音
再举一个 : 开发者总是喜欢让 agent 的 tool use 完整的展示到前端,但是背后真正的用户体验逻辑是我需要知道 agent 在工作/在往哪个方向偏,所以我们并不需要如此详细的信息

设计哲学：认知负担,信息,价值导向,美学

换个话题
我一个人做 用了三个月,现在他能用了,但是它离完美很远
做产品很重要的一件事是…不要闭门造车,也就是我需要你们的真诚建议

引用来自 readme 中的一句话
“我欢迎所有形式的贡献。即使你不是开发者，只是一个普通用户——如果你在使用中感到任何不舒服的地方，哪怕只是一点间距、一个颜色、一个很小很小的细节，或者是一个很大的方向，都可以直接开一个 issue。
我认真对待每一个体验细节，你的反馈会让所有人的体验变得更好。
如果你是开发者， 的 Agent 核心、记忆系统、hook 机制都是开放的。你可以接自己的 provider、写自己的插件、甚至改掉你不喜欢的任何设计。”这是我认为整个项目最精髓的一点,我希望看到你们的反馈,不管是细节还是方向
我在用 dify 的时候,我常常发现,一个特别小的间距问题,竟然在这么大的仓库里没人修
因此,我很重视这点理念

现在并不稳定，还有很多不完善的地方。希望大家多多包容,多提意见

另带一提, 可以从 openclaw/hermes 导入配置

首先，我觉得你一个人打磨三个月，我是非常佩服你的。

但是我想问一下，你有没有跟我一样的一个痛点？我刚在 DimCode 那边说了一遍：当我们这个对话聊到 100 万上下文的时候，请告诉我这个对话应该如何导航。

我想到中间去看一看，我怎么知道中间发生了什么？我怎么能一下子跳过去，看一看当初 AI 说了什么，当初我说了什么？这件事到底是怎么解决的？为什么没有人去解决？

你们发明了一个又一个（实话说长得非常相似的）东西，不同的 session 都可以被打开，但是 session 之内的不同内容却无法被导航，认真的吗？真的需要做一个又一个不解决用户痛点的东西吗？

其实我一直在想接入一下 老师的 Flowchat那个项目，把它的设计理念带到arkloop来，也许在之后我会做这个feature，我认为这个项目的理念可以解决这个问题

先加一个VS Code一样的代码导航，就是minimap，我觉得这能直接解决很大的问题，至于说用side agent做的目录索引，可以慢慢加。

---

前段时间，派出所 给咱打了电话，问咱有没有买过 无人机。
咱：没有，家里人也没有。
派出所：彳亍。如果你买了无人机，记得去 UOM 平台实名登记。

据说派出所会根据网购平台购买记录筛查目标。咱翻了翻某东和某宝，还真发现了几条购买记录：
「三星内存卡 256G TF 卡 microsd 卡大疆相机无人机高速 Switch 存储卡……」
「2/3 串 3.7V 快充锂电池均衡无人机充电模块大功率……」
警察同志，您多虑了，这种东西可不会飞（）

#吐槽

---

现在 ai ctf 时代了你告诉我要选拔了
早几把干啥去了

和软著的 AI 成分一样难绷

这又是给谁家公子哥挖的新路

我不是有意针对谁哈，就是看到了转一下 😄
可能有少爷看上安全了吧

少爷居然也有个黑客梦

---

机器人想知道爱是什么

什么叫做爱

做爱

守身如玉

---

有一段时间没更新了，对此想法是 我也想当可切换线程生物。
自己因为线程切不过来，一天只能安排两三项任务，这样就很难维持有规划的生活。

毕竟有规划的生活基本都有很多每日任务，像什么游戏日常，背单词，更新小说。
这就要求在某个任务没完成之前频繁切换线程。

但单线程生物就做不到，必须排队执行，先完成其中一个。
某种意义上来说也是解释三分钟热度的一种借口吧，不能及时达到目标。
有了其他目标或防止自己卡死只能丢弃这部分进度了，至于什么一天学半个小时自然是做不到的。

---


