...
let voyage = {
  ...
  create(...options) {
    const { is, isnt, check, has, lacks, init, each, bind } = voyage;
    const { assign: give } = Object;
      ...
      createNode(element) {
        ...
        for (const label in labels) {
          ...
          if (is(label, "class") && check(content, Array)) {
            ...
            const { componentid, stateid } = content;
            bind(componentid, stateid, labelUpdater(node, label));
          } else {
