...
let voyage = {
  ...
  lib: {
     ...
     * @param  {Array<string|SingleElementArray<string>|object>} path - path to be init
     ...
     * @returns {SingleElementArray|false} an array containing the prop
     ...
     * @param {sync-observer} observer - the callback fn when obj[key] changes
     * @returns {sync-reactive} the reactive object
     * @memberof voyage.lib
     */
  ...
  get(...options) {
    ...
    const { memorize, getReactiveState } = voyage;
    ...
    if (is(options.length, 1)) {
      ...
      const { memo } = voyage;
      return memo.getReactiveState[componentid];
    } else {
      const [componentid, stateid] = options;
      return memorize(getReactiveState, componentid, stateid);
    }
  },
  getReactiveState(componentid, stateid) {
    const { sync, use } = voyage.lib;
    const { updateState } = voyage;
    const { assign: give } = Object;

    const { states } = voyage;
    const reactiveState = sync(
      states[componentid],
      stateid,
  ...
  lookup(key) {
    ...
    const reactiveState = memorize(getReactiveState, "global", key);
  ...
  ref(...options) {
    ...
    const { refState } = {
      refState(initial) {
        ...
        const reactiveState = memorize(getReactiveState, componentid, stateid);
        return reactiveState;
      },
    };
  ...
  keep(obj) {
    ...
    const { keepGlobal } = {
      keepGlobal(key, value) {
        ...
        return memorize(getReactiveState, "global", key);
      },
    };
